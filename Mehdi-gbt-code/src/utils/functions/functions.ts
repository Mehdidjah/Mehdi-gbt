 ...
 */
export function processTextAndCodeBlocks(deltas: CompletionChunk[] | string) {
	const content = typeof deltas === 'string' ? deltas : (Array.isArray(deltas) ? deltas.join('') : String(deltas));
	...
	if (lastIndex < content.length) {
		...
		if (remainingText) {
			blocks.push({
				type: 'text',
				content: ' ' + remainingText.replace(/\n/g, '<br>') + ' '
			});
		}
	}

	// If no blocks were found, return the entire content as text
